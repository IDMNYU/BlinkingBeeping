...
We want the LED to blink when the button is pressed, that is, the voltage on the pin is `HIGH`. We can test this condition with an `if()` statement. `if()` evaluates a statement to see if it is true or false. If the condition is met (true) it will execute the code in the curly brackets that follow it. If not, it will ignore that code.
