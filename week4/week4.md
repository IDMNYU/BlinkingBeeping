# Notes on Digital Input and Output 
...
[Read this to get a good idea of the ins and outs of the Arduino IDE for your desktop.](https://www.arduino.cc/en/Guide/Environment)  It's also possible to use a cloud based IDE, a commandline based IDE, or an older version. We're going to use the 2.x branch in this course.

The [Arduino microcontroller we’ll be using in class is called the Nano BLE 33](https://store-usa.arduino.cc/products/arduino-nano-33-ble). [Thanks to it’s open hardware philosophy](https://www.arduino.cc/en/Main/Policy) (like open source software, but for physical objects) there are as many Arduino clones as there are stars in the sky. This has also cemented its place in the media art world as the cornerstone around which physical things are made. There are a bunch of variants (Uno, MKR1000, Micro, Due, Zero, Lilypad, Gemma, Ethernet, etc etc), each with a particular specialty. This board has built-in bluetooth, an acceleromteter, and a gyroscope. You can also run some machine learning models on here - we'll get to all of this in the coming weeks.
...
Before we do the “Hello World” of hardware and get an LED to blink, we need to install the board definitions for the Nano BLE 33. In the IDE, select the Tools > Boards > Boards Manager. Search for 'Arduino Mbed OS Nano Boards' and press the Install button. Once it has installed, you can close the board manager.

![board manager with  Arduino Mbed OS Nano Boards definitions](https://docs.arduino.cc/static/7b971f21a2676a06b2f13cd4770635ae/29114/install_mbed_img03.png)
...
![LED and Arduino on a breaboard](LED_output.png)
...
![LED Switch and Arduino on a breaboard](LED_Switch.png)
...
2 LEDs, 1 switch
![2 LEDs, a Switch and Arduino on a breaboard](2LED_1switch.png)

2 LEDs, 2 switches (for use with the and/or program)
![2 LEDs, 2 Switches and Arduino on a breaboard](2LED_2switch.png)